...
import numpy as np
import h5py
from utils.model_utils import get_dataset_name, RUNCONFIGS, get_parameters
import copy
...
class Server:
    ...
    def add_parameters(self, user, ratio, partial=False):
        if isinstance(self.model, dict):
            for server_param, user_param in zip(get_parameters(self.model), get_parameters(user.model)):
                for i in range(len(user_param)):
                    server_param[i].data = server_param[i].data + user_param[i].data.clone() * ratio
        else:
            if partial:
    ...
    def save_results(self, args):
        ...
        with h5py.File("./{}/{}.h5".format(self.save_path, alg), 'w') as hf:
            for key in self.metrics:
                # if isinstance(self.metrics[key][0][0], torch.Tensor):
                #     for i in range(len(self.metrics[key])):
                #         for j in range(len(self.metrics[key][i])):
                #             self.metrics[key][i][j] = self.metrics[key][i][j].cpu().detach().numpy()
