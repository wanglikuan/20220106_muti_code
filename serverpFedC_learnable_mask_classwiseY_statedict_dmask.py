...
class pFedC(Server):
    def __init__(self, args, model, seed):
        ...
        #初始化-输入tensor P
        self.input_P = {}
        for mu_index in range(total_users):
            self.input_P[mu_index] = {}
            for i in range(self.num_classes):
                #self.input_P[i] = torch.rand(self.num_classes, 2, requires_grad=True)
            #    
                self.input_P[mu_index][i] = torch.ones(total_users, 2, requires_grad=True)
                self.input_P[mu_index][i] = self.input_P[mu_index][i].float()/2
                self.input_P[mu_index][i] = self.input_P[mu_index][i].cuda()
                self.input_P[mu_index][i].retain_grad()
        #
        #初始化-输出mask Y
        self.output_Y = {}
        for mu_index in range(total_users):
            self.output_Y[mu_index] = {}
            for i in range(self.num_classes):
                self.output_Y[mu_index][i] = torch.zeros(total_users, requires_grad=True)
                self.output_Y[mu_index][i] = self.output_Y[mu_index][i].cuda()
        #
    ...
    def my_gumbel_softmax(self):
        for mu_index in range(self.users_num):
            for i in range(self.num_classes):
                for index in range(self.users_num):
                    tmp_tensor = F.gumbel_softmax(self.input_P[mu_index][i][index],tau=0.1,hard=True)
                    self.output_Y[mu_index][i][index] = tmp_tensor[0]
    ...
    def update_masks(self):
        for mu_index, mu in enumerate(self.selected_users):
            ...
            for id, (param, local_param) in enumerate(zip(get_parameters(mu.personalized_model), get_parameters(mu.model))):
                if id == 0: #'rep'
                    continue
                elif id > 0 and self.output_Y[mu_index][id-1][mu_index] == 1: #client 自身有此类label
                    ...
                    for k, tensor in origin_state.items():
                        for key in flag_list:
                            if k == key:
                                ...
                                break 
                    print("------------------------------------------------------------")
                    print("mu_index: ",mu_index," id: ",id)
                    #print("origin_state_list[0]: ",origin_state_list[0])
                    #print("delta_attention_theta_list[0]: ",delta_attention_theta_list[0])
                    print("self.input_P[mu_index][id-1]: ",self.input_P[mu_index][id-1])
                    print("self.output_Y[mu_index][id-1]: ",self.output_Y[mu_index][id-1])

                    tmp_i_grads = torch.autograd.grad(
                        origin_state_list, self.input_P[mu_index][id-1], grad_outputs=delta_attention_theta_list, retain_graph=True, allow_unused=True
                    ) 

                    print("Mask_grads:", tmp_i_grads)
                    if tmp_i_grads[0] is None:
                        continue
                    #print("self.input_P[mu_index][id-1].grad", self.input_P[mu_index][id-1].grad) #none


                    #print("len(self.input_P[mu_index][id-1]): ",len(self.input_P[mu_index][id-1])) #20 clients_num
                    for i in range(len(self.input_P[mu_index][id-1])):
                        #print("step1 self.input_P[mu_index][id-1][i]: ",self.input_P[mu_index][id-1][i])
                        #print("mask_grad: ",tmp_i_grads[0][i])
                        self.input_P[mu_index][id-1][i] = self.input_P[mu_index][id-1][i] - tmp_i_grads[0][i] * grad_lr
                        #print("step2 self.input_P[mu_index][id-1][i]: ",self.input_P[mu_index][id-1][i])

                elif id > 0 and self.output_Y[mu_index][id-1][mu_index] == 0: #client 自身没有此类label
                    continue
    ...
    def evaluate(self, save=True, selected=False):
        ...
        #print出每个client的平均acc
        print("##################################################################")
        print("test_samples: ",test_samples)
        print("test_accs: ",test_accs)
        print("each client's acc:")
        clients_accs = []
        for u in range(self.num_users):
            tmp_u_acc = 0.0
            for i in range(self.num_classes):
                tmp_u_acc += test_accs[u][i]
            tmp_u_acc = tmp_u_acc/self.num_classes
            clients_accs.append(tmp_u_acc)
            print(tmp_u_acc)
        print("##################################################################")
        #
            ...
            for mw_index, mw in enumerate(self.selected_users):                
                for id, (param, local_param) in enumerate(zip(get_parameters(mu.personalized_model), get_parameters(mw.model))):
                    if id == 0: #'rep'
                        ...
                        if mu_index == 0 :
                            print("AGG - client 0 id ",id," = 1", "  mw_index = ", mw_index)
                            #print("len(param): ",len(param))
                        if self.output_Y[mu_index][id-1][mw_index] == 0: #轮询的client没有此类label，跳过
                            # for j in range(len(param)): 
                            ...
                            for j, layer_name in enumerate(flag_list):
                                ...
                                self.personalized_state_dicts[mu_index][id-1][layer_name] += self.output_Y[mu_index][id-1][mw_index] * local_param[j].data.clone() * (mw.train_samples / total_train)                           
                            continue
                        #--------------------------------------------------------------------------------------------
                    ...
                    elif id > 0 and self.output_Y[mu_index][id-1][mu_index] == 0: #client 自身没有此类label，则就使用自身参数
                        if mu_index == 0 :
                            print("AGG - client 0 id ",id," = 0" , "  mw_index = ", mw_index)                        
                        for j in range(len(param)):
                            param[j].data += original_params[id][j].data.clone()
            #---------------------------------------------------------------------------------------------------------------------------------------------
